...
页面的横轴是年代，下方是基督教传教大事件，上方是对应时期的中国历史大事件。点击事件卡片，可以查看该事件的更详细内容。
...
每个事件都是一个文件夹，文件夹名是事件名 `年份-英文短名` 例如：“马礼逊抵达广州”是 `1807-morrison-canton`。具体内容写在该文件夹下的 `index.md`中，没有 `index.md` 的事件不会在网页中显示。格式如下：
...
图片等资源与事件文档 `index.md` 在同一文件夹中，正文引用图片，使用相对路径：`![马礼逊像](robert-morrison.jpg)`。